...
def extract_year_from_text(text):
    ...
    # Pattern 3: FOR THE PERIOD : DD Mon YYYY
    match = re.search(r'FOR\s+THE\s+PERIOD[:\s]+\d{1,2}\s+[A-Za-z]+\s+(\d{4})', text, re.IGNORECASE)
    if match:
        return match.group(1)
    
    # Pattern 4: DD Mon YYYY (in text)
    match = re.search(r'\d{1,2}\s+(Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)[a-z]*\s+(\d{4})', text, re.IGNORECASE)
    if match:
