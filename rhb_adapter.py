...
def parse_transactions_rhb(pdf, source_file):
    ...
    m = re.search(r"\d{1,2}\s+[A-Za-z]{3}\s+(\d{2})", header_text)
    if not m:
    ...
    for page_no, page in enumerate(pdf.pages, start=1):
        ...
        for line in lines:
            ...
            if "B/F" in line.upper() or "C/F" in line.upper():
                continue
            ...
            for w in line_words.get(line, []):
                ...
                if num_re.fullmatch(txt):
                    nums.append((float(txt), (w["x0"] + w["x1"]) / 2))
            ...
            # 2) Fallback → rightmost number
            if balance is None and nums:
                balance = nums[-1][0]
